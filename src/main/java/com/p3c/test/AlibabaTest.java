 ...
 *
 * <p>11，和数据库相关的单元测试，可以设定自动回滚机制，不给数据库造成脏数据。或者对单元测试产生的数据有明确的前后缀标识。
 *
 * <p>12，对不可测的代码建议做必要的重构，使代码变得可测，避免为了达到测试要求而书写不规范的测试代码。
 *
 * <p>13，在测试评审阶段，开发人员需要和测试人员一起明确单元测试范围，单元测试最好覆盖所有的测试用例。
 *
 * <p>14，单元测试是一种质量保障手段，不建议项目发布后补充单元测试用例，建议在项目前完成单元测试。
 *
 * <p>15，为了更方但的进行单元测试，业务 代码应出现以下情况： 1）构造方法中做的事情过多。 2）存在过多的全局变量和静态方法 3）存在过多的外部依赖。 4）存在过多的条件分支
 * 多层条件语句建议使用卫语我，在策略模式，状态模式等进行重构。
 *
 * <p>16，不要对单元测试存在如下误解， 1）那是测试工程师干的事情，本文是开发手册，凡是本文内容都与开发工程师强相关的 2）单元测试是多余的，汽车的整体功能与种单部件的测试正确与否都是强相关的
 * 3）单元测试不需要维护，如不维护，1年半载后，单元测试几乎处于废弃状态。 4）单元测试与线上故障没有辩证关系，好的单元测试能够最大限度的规避线上故障.
 *
 * @author liujun
 * @version 0.0.1
 * @date 2018/09/03
